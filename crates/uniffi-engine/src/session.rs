...
struct LifecycleGate {
  env: sys::napi_env,
  /// N-API references and Host hooks are only valid on the thread that owns
  /// the environment.  Keep the marker in the shared gate so callback
  /// invokers can reject an off-thread returned-callback retain before they
  /// resolve the state pointer or touch the raw environment.
...
impl LifecycleGate {
  ...
  fn ensure_owner_thread(&self) -> Result<()> {
    if self.is_owner_thread() {
      ...
      Err(Error::new(
        Status::GenericFailure,
        "UniFFI callback invoker must run on the owning JavaScript thread",
      ))
    }
  }
...
impl SessionCallbackInvoker {
  ...
  /// invocation ID.  Synchronous callback methods use this guard on every
  /// entry; unlike `next_invocation_id`, it intentionally leaves the shared
  /// ID domain untouched.
  ...
  /// Return whether the current thread owns this session's JavaScript
  /// environment. This performs only the immutable thread-marker check; it
  /// never resolves SessionState or touches an N-API reference. Callers must
  /// still use `check_open`/`with_host` before entering the Host.
  ...
  ///
  /// The callback receives borrowed handles that are valid only for the
  /// duration of this call.  In particular, callers must not move either
  /// value into a proxy/future or store their raw N-API pointers.  The Host
  /// comes from the SessionState's persistent reference, so this remains
  /// valid after a per-invocation host proxy has been finalized.
  pub fn with_host<R>(&self, callback: impl FnOnce(&Env, &Object<'static>) -> R) -> Result<R> {
    ...
    // Safety: `state` is published by `install_state` and is cleared before
  ...
  /// Retain a callback returned by a callback method while still on the
  /// owning JS thread.  The invoker resolves the live session through the
  /// lifecycle gate only after the open/thread checks pass, then delegates to
  /// the one SessionState callback lease registry and release queue.  The
  /// returned lease is pure Rust/Arc state and may safely cross threads; no
  /// raw N-API environment, reference, or value escapes this method.
  pub fn retain_returned_callback(
    ...
    // Safety: `state` is published by `install_state` and is cleared before
    // lifecycle teardown releases any state-owned N-API references.  The
    // owner-thread/open checks above prevent dereferencing a revoked state;
...
impl SessionState {
      ...
      for value in values {
        match segment {
          ...
          SessionValuePathSegment::SequenceElement => {
            ...
            napi::check_status!(unsafe { sys::napi_is_array(self.env, value, &mut is_array) })?;
            if !is_array {
              return Err(Error::new(
                Status::InvalidArg,
  ...
  fn new_callback_lease(&self, key: CallbackKey) -> Result<SessionCallbackLease> {
    ...
    // re-entrant close can therefore claim and drain this token while
    // `retainCallback` is running; the post-hook check below then observes a
    // revoked invoker without publishing a second release.
  ...
  fn retain_argument_resources(
    ...
    Ok(RetainedInvocation { transfer_id })
  }
...
fn create_callback_release_tsfn(
  ...
  if let Err(error) = napi::check_status!(unsafe {
  ...
  if let Err(error) = napi::check_status!(unsafe { sys::napi_unref_threadsafe_function(env, tsfn) })
  {
    let _ = unsafe {
      sys::napi_release_threadsafe_function(tsfn, sys::ThreadsafeFunctionReleaseMode::abort)
    };
...
fn value_u32(env: sys::napi_env, value: sys::napi_value, role: &str) -> Result<u32> {
  ...
  let mut number = 0.0;
  napi::check_status!(unsafe { sys::napi_get_value_double(env, value, &mut number) })?;
...
/// Resolve one branch of the canonical output-stream step carrier.  The
/// carrier is intentionally validated before branch selection: a mismatched
/// valid branch is skipped, while a malformed object is a protocol error that
/// must abort tracking and trigger the caller's existing rollback path.
...
/// hidden property. Symbols are deliberately excluded: the canonical DTO
/// contract has only the string keys listed by `stream_step_payload`.
fn own_string_keys(env: sys::napi_env, object: sys::napi_value) -> Result<BTreeSet<String>> {
  ...
  let mut length = 0;
  napi::check_status!(unsafe { sys::napi_get_array_length(env, properties, &mut length) })?;
...
fn iterable_values(
  ...
  loop {
    ...
    napi::check_status!(unsafe { sys::napi_get_value_bool(env, done, &mut is_done) })?;
    if is_done {
      break;
    }
...
fn iterable_entries(
  ...
  for entry in entries {
    let mut is_array = false;
    napi::check_status!(unsafe { sys::napi_is_array(env, entry, &mut is_array) })?;
    if !is_array {
      return Err(Error::new(
        Status::InvalidArg,
    ...
    let mut key = ptr::null_mut();
    let mut value = ptr::null_mut();
    napi::check_status!(unsafe { sys::napi_get_element(env, entry, 0, &mut key) })?;
    napi::check_status!(unsafe { sys::napi_get_element(env, entry, 1, &mut value) })?;
...
fn resolve_variant(
  ...
  let mut value_type = sys::ValueType::napi_undefined;
  napi::check_status!(unsafe { sys::napi_typeof(env, discriminant, &mut value_type) })?;
...
fn is_thenable(env: sys::napi_env, value: sys::napi_value) -> Result<bool> {
  ...
  napi::check_status!(unsafe { sys::napi_typeof(env, then, &mut then_type) })?;
